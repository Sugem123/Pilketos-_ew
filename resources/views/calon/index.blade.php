...
<x-app-layout :page_title="$page_title" :page_description="$page_description">
            ...
            <div class="divide-y divide-gray-100 flex-1 overflow-y-auto">
                @forelse($calons as $calon)
                    <div onclick="selectCandidate({{ $calon->id }})" id="row-{{ $calon->id }}"
                        ...
                        <div
                            class="w-12 h-12 rounded-xl bg-gradient-to-br from-gray-100 to-gray-200 flex items-center justify-center overflow-hidden flex-shrink-0">
                            @if ($calon->url_foto)
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center gap-2 mb-0.5">
                                <span
                                    class="w-5 h-5 rounded-full bg-birupesat/10 text-birupesat text-xs font-bold flex items-center justify-center flex-shrink-0">{{ $calon->nomor }}</span>
                                <h3 class="font-semibold text-accent text-sm truncate">{{ $calon->nama }}</h3>
                            </div>
                            <p class="text-xs text-gray-400">{{ $calon->kelas->name }}</p>
                        </div>
                        <i
                            class="fas fa-chevron-right text-gray-300 text-xs group-hover:text-accent transition-colors"></i>
                    </div>
                @empty
        ...
        <div class="lg:col-span-3 bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden flex flex-col">
            ...
            <div id="detail-content" class="hidden flex-1 flex flex-col">
                ...
                <div class="p-6 flex-1 overflow-y-auto">
                    {{-- Layout: Photo (left) | Identity + Visi + Misi (right) --}}
                    <div class="flex gap-6">
                        {{-- Photo --}}
                        <div id="detail-foto-wrap"
                            class="w-64 h-90 rounded-2xl bg-gradient-to-br from-gray-100 to-gray-200 flex items-center justify-center overflow-hidden flex-shrink-0 shadow-sm">
                            <img id="detail-foto" src="" alt=""
                                class="w-full h-full object-cover hidden">
                            {{-- <i id="detail-foto-placeholder" class="fas fa-user text-gray-400 text-5xl hidden"></i> --}}
                        </div>
                        ...
                        <div class="flex-1 flex flex-col gap-5">
                            ...
                            <div>
                                <div class="flex items-center gap-2 mb-2">
                                    <span id="detail-nomor"
                                        class="px-2.5 py-0.5 bg-birupesat/10 text-birupesat text-xs font-bold rounded-full"></span>
                                </div>
                            ...
                            <div>
                                ...
                                <p id="detail-visi"
                                    class="text-sm text-gray-600 leading-relaxed bg-gray-50 rounded-xl p-4"></p>
                            </div>
                            ...
                            <div>
                                ...
                                <p id="detail-misi"
                                    class="text-sm text-gray-600 leading-relaxed bg-gray-50 rounded-xl p-4 whitespace-pre-line">
                                </p>
                            </div>
                        </div>
                    </div>
    ...
    <div id="secondary-sidebar"
        class="fixed inset-y-0 right-0 w-full sm:w-[480px] bg-white shadow-2xl z-50 transform translate-x-full transition-transform duration-300 ease-in-out flex flex-col pointer-events-none">
                ...
                <div>
                    ...
                    <div class="flex gap-4">
                        <div id="preview-container"
                            class="hidden w-24 h-24 rounded-lg border border-gray-200 overflow-hidden flex-shrink-0"
                        ...
                        <label for="foto-input"
                            class="flex-1 flex flex-col items-center justify-center py-8 border-2 border-dashed border-gray-300 rounded-xl text-sm text-gray-400 hover:border-accent hover:text-accent cursor-pointer transition-all">
